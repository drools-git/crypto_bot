...
class CoinGeckoProvider(BaseProvider):
    ...
    async def _fetch_ohlcv_impl(self, symbol: str, timeframe: str, limit: int) -> List[OHLCV]:
        formatted_symbol = symbol.replace("USDT", "/USD") if "USDT" in symbol and "/" not in symbol else symbol
        data = await self.exchange.fetch_ohlcv(formatted_symbol, timeframe, limit=limit)
        return [OHLCV(time=c[0]//1000, open=c[1], high=c[2], low=c[3], close=c[4], volume=c[5] if len(c) > 5 else 0.0) for c in data]
